...
const initialRows: TableItem[] = Array.from({ length: 12 }).map((_, idx) => ({ id: String(idx + 1), name: '02 Inventory System item ' + (idx + 1), owner: idx % 2 ? 'alice' : 'bob', status: idx % 3 ? 'active' : 'pending', updatedAt: new Date(Date.now() - idx * 3600_000).toLocaleDateString() }));
...
export default function App() {
  ...
  return (
    <SaasLayout appName="02 Inventory System" nav={nav} dark={dark} toggleDark={toggleDark} onSearch={setQuery}>
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2"><h1 className="text-xl md:text-2xl font-semibold">02 Inventory System Dashboard</h1><button onClick={pingBackend} className="rounded-md border border-slate-700 px-3 py-2 text-sm">Check API</button></div>
